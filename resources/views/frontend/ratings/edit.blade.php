    ...
    <div class="row justify-content-center">
        <div class="col-md-12">
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <form method="POST" action="{{ route("frontend.ratings.update", [$rating->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group">
                            ...
                            <select class="form-control select2" name="movie_id" id="movie_id">
                                @foreach($movies as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('movie_id') ? old('movie_id') : $rating->movie->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                            @if($errors->has('movie'))
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control select2" name="user_id" id="user_id">
                                @foreach($users as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('user_id') ? old('user_id') : $rating->user->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                            @if($errors->has('user'))
                                <div class="invalid-feedback">
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control" name="rate" id="rate">
                                ...
                                @foreach(App\Models\Rating::RATE_SELECT as $key => $label)
                                    <option value="{{ $key }}" {{ old('rate', $rating->rate) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
                            @if($errors->has('rate'))
                                <div class="invalid-feedback">
                        ...
                        <div class="form-group">
                            ...
                            <input class="form-control" type="text" name="review" id="review" value="{{ old('review', $rating->review) }}">
